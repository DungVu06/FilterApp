                ...
                for box, face_id in zip(boxes, ids):
                    ...
                    if face_id not in face_filters:
                        face_filters[face_id] = {
                            "x": [OneEuroFilter(t_now, lx * box_w + x1, min_cutoff=0.005, beta=0.1) for (lx, ly) in landmarks],
                            "y": [OneEuroFilter(t_now, ly * box_h + y1, min_cutoff=0.005, beta=0.1) for (lx, ly) in landmarks],
                            "prev_smoothed": np.zeros((98, 2), dtype=int),
                            "last_seen": t_now
                        }
                    else:
                    ...
                    for i, (lx, ly) in enumerate(landmarks):
                        ...
                        # one euro
                        smooth_x = int(face_filters[face_id]["x"][i].apply(t_now, raw_x))
                        smooth_y = int(face_filters[face_id]["y"][i].apply(t_now, raw_y))

                        prev_pt = face_filters[face_id]["prev_smoothed"][i]
